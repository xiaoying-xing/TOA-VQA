...
from .interpolate import interpolate_pos_embed, interpolate_relative_pos_embed
...
def build_text_encoder(config, vision_width, load_text_params=False, use_mlm_loss=False, config_text=None):
    ...
    config_text = BertConfig.from_json_file('models/xvlm/config_bert.json')
...
def load_pretrained(ckpt_rpath, config, is_eval=False, load_text=False):
    ...
    if config['use_clip_vit']:
        ...
        with open(config['vision_config'], 'r') as f:
...
class XVLMModel():
    ...
    def __init__(self, device, path_checkpoint='models/xvlm/pretrained_models/checkpoint_9.pth'):
        self.dev = device
        image_res = 384
        self.max_words = 30
        self.thresh = 0.6
        ...
        #with open('random_negatives.txt') as f:
        #    self.negative_categories = [x.strip() for x in f.read().split()]
        self.possible_options = json.load(open('possible_options.json'))
    ...
    def binary_score(self, image, text=None, negative_categories=None):
        ...
        if text:
            ...
            texts = negative_categories
            sim = 100 * self.score(image, texts)[0]
            return torch.argmax(sim), torch.max(sim)

    # Can we add a confidence to the result?
    def filter(self, image, object, attribute, negative=True):
        if isinstance(image, str):
            image = Image.open(image).convert('RGB')
        image = transforms.PILToTensor()(image)
        name = f"{attribute} {object}"
        if negative:
            negative_categories = [f"{att} {object}" for att in self.possible_options['attributes']]
            res = self.binary_score(image, name, negative_categories)
        else:
            res = self.score(image, name)

        res = res.cpu().item()
        return str(res > self.thresh)

    def query_attribute(self, image, object, attribute_class):
        if isinstance(image, str):
            image = Image.open(image).convert('RGB')
        image = transforms.PILToTensor()(image)
        if attribute_class+'s' in self.possible_options.keys():
            candidates = [f"{att} {object}" for att in self.possible_options[attribute_class+'s']]
            idx, score = self.binary_score(image, negative_categories=candidates)
            idx, score = idx.item(), score.item()
            concept = list(self.possible_options[attribute_class + 's'])[idx]
        else:
            candidates = [f"{att} {object}" for att in self.possible_options['attributes']]
            idx, score = self.binary_score(image, negative_categories=candidates)
            idx, score = idx.item(), score.item()
            concept = list(self.possible_options['attributes'])[idx]
